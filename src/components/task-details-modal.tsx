      ...
      <div role="dialog" aria-modal="true" aria-labelledby="task-details-title" className="w-full max-w-lg rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl shadow-black/40">
        <form onSubmit={handleSubmit} onKeyDown={(event) => event.key === "Escape" && close()}>
          <header className="flex items-center justify-between gap-4 border-b border-zinc-800 px-5 py-4">
            <h2 id="task-details-title" className="flex min-w-0 flex-1 items-center gap-2 text-lg font-semibold">
              <button
                ...
                title={`Mark as ${completed ? "active" : "completed"}`}
              ...
              {isEditingTitle ? (
                ...
                <button
                  ...
                  className={`min-w-0 flex-1 truncate rounded px-1 text-left hover:bg-zinc-800 ${completed ? "text-zinc-400 line-through" : ""}`}
                >
                  {title}
                </button>
              )}
            </h2>
            <button type="button" onClick={close} disabled={isSaving} aria-label="Close" className="grid size-8 shrink-0 place-items-center rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 disabled:opacity-40">
          ...
          <div className="space-y-4 px-5 py-5">
            <div>
              <label htmlFor="edit-task-notes" className="mb-1.5 block text-sm font-medium text-zinc-300">Notes <span className="font-normal text-zinc-500">(optional)</span></label>
              <textarea id="edit-task-notes" rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Add any details" className="w-full resize-none rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-500 focus:border-zinc-500" />
            </div>
            <div>
              <label htmlFor="edit-task-due-date" className="mb-1.5 block text-sm font-medium text-zinc-300">Due date <span className="font-normal text-zinc-500">(optional)</span></label>
          ...
          <footer className="flex justify-end gap-2 border-t border-zinc-800 px-5 py-4">
            <button type="button" onClick={close} disabled={isSaving} className="rounded-lg px-3 py-2 text-sm font-medium text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 disabled:opacity-40">Cancel</button>
            <button type="submit" disabled={!title.trim() || isSaving} className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-zinc-950 hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-40">{isSaving ? "Saving…" : "Save changes"}</button>
          </footer>
        </form>
      </div>
    </div>
